...
public class PrepaidDBController extends DBConnector {
	/* static values */
	public static String DB_NAME = "labshop";
	public static String COLLECTION_NAME = "prepaid";
	public static String COUNTER_KEY = "prepaid";
	public static int COUNTER_MIN = 10000000;
	public static int COUNTER_MAX = 99999999;
	public static int IDGEN_OVERFLOW = 1000;

	/* static methods */
	public static Document toDocument(PrepaidData data){
		Document doc;
		doc = new Document()
	...
	public static PrepaidData toPrepaidData(Document doc){
		if( doc != null ){
	...
	public static List<PrepaidData> toPrepaidData(FindIterable<Document> list){
		List<PrepaidData> datas = new ArrayList<PrepaidData>();
		for(Document doc : list){
			datas.add(toPrepaidData(doc));
		}
		return datas;
	}
	...
	public PrepaidDBController(){
		super();
		initCollection();
	}

	public PrepaidDBController(String host, int port){
		super(host, port);
		initCollection();
	}

	public PrepaidDBController(DBConnector c0){
		super(c0);
		initCollection();
	}
	...
	public List<PrepaidData> getAllPrepaidList(){
		FindIterable<Document> result = collection.find();

		return toPrepaidData(result);
	}

	public PrepaidData addPrepaid(PrepaidData acc, boolean auto_num, boolean auto_stamp){
		if( auto_num ){
	...
	public boolean updatePrepaid(PrepaidData data){
	...
	public boolean deletePrepaid(int acc_id){
	...
	public PrepaidData publishUniqueIdPrepaid(){
		PrepaidData acc = new PrepaidData();
		acc.setId(popCounter());

		return acc;
	}

	public PrepaidData getPrepaidById(int acc_id){
		return toPrepaidData(
	...
	public void refreshCounter(){
		...
		if( max_id >= COUNTER_MAX ){
			System.err.println("FATAL: ID GENERATE ERROR (in Prepaiddbcontroller)");
		}
